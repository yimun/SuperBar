...
public class InputCmd {
    ...
    /*public static void main(String[] args) {
        (new InputCmd()).run(args);
    }*/

    private static InputCmd ins;
     ...
     * @param point
     */
    public static void runTap(Point point){
    	if(ins == null)
    		ins = new InputCmd();
    	ins.inputSource = getSource(ins.inputSource, InputDevice.SOURCE_TOUCHSCREEN);
    	ins.sendTap(ins.inputSource, Float.valueOf(point.x),
                 Float.valueOf(point.y));
    }
    /**
     * 滑动事件注入
     * @param from
     * @param to
     */
    public static void runSwipe(Point from,Point to){
    	if(ins == null)
    		ins = new InputCmd();
    	int duration = -1;
        ins.inputSource = getSource(ins.inputSource, InputDevice.SOURCE_TOUCHSCREEN);
        ins.sendSwipe(ins.inputSource,
                Float.valueOf(from.x), Float.valueOf(from.y),
                Float.valueOf(to.x), Float.valueOf(to.y),
                duration);
    }
    /**
     * 按键事件注入
     * @param keyCode
     */
    public static void runKeyEvent(int keyCode){
    	if(ins == null)
    		ins = new InputCmd();
        ins.sendKeyEvent(ins.inputSource, keyCode, false);
    }
